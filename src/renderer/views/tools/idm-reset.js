...
function init() {
  ...
  if (!resetBtn || !statusEl || !checkBtn || !progressEl || !stepsEl) {
    console.error('[IDM Reset] Missing elements!', {
      resetBtn, statusEl, checkBtn, progressEl, stepsEl
    });
    return;
  }
  ...
  function showStep(step, status = 'running') {
    ...
    // Set trạng thái cho step hiện tại
  ...
  // Kiểm tra trạng thái IDM
  checkBtn.addEventListener('click', async () => {
    if (isCheckingIDM) return; // Prevent double click
    isCheckingIDM = true;
    
    console.log('[IDM Reset] Check button clicked');
    ...
    try {
      ...
      if (result.running) {
        ...
        if (result.daysLeft !== null && result.daysLeft !== undefined) {
          statusText += ` • Còn ${result.daysLeft} ngày trial`;
          toastText += ` • Còn ${result.daysLeft} ngày trial`;
        } else if (result.hasRegistry) {
          statusText += ' • Trial (không xác định được số ngày)';
  ...
  resetBtn.addEventListener('click', async () => {
    ...
    console.log('[IDM Reset] Reset button clicked');
    
    const confirmed = confirm(
      'Xác nhận reset IDM trial\n\n' +
      'Thao tác này sẽ:\n' +
      '• Tắt IDM nếu đang chạy\n' +
      '• Xóa registry keys\n' +
      '• Xóa folder cấu hình\n\n' +
      'Bạn có chắc chắn?'
    );
    ...
    showStep(1, 'running');
    statusEl.textContent = '⏳ Đang tắt IDM processes...';
    statusEl.className = 'idm-status checking';
    ...
    try {
      ...
      if (result.ok) {
        ...
        // Bước 2: Registry
        showStep(2, 'running');
        statusEl.textContent = '⏳ Đang xóa registry keys...';
        await new Promise(resolve => setTimeout(resolve, 500));
        showStep(2, 'success');
        await new Promise(resolve => setTimeout(resolve, 300));
        ...
        // Bước 3: Folder
        showStep(3, 'running');
        statusEl.textContent = '⏳ Đang xóa folder cấu hình...';
        await new Promise(resolve => setTimeout(resolve, 500));
        showStep(3, 'success');
        await new Promise(resolve => setTimeout(resolve, 300));
        ...
        // Thành công
        statusEl.textContent = '✓ Reset thành công! Khởi động lại IDM để áp dụng.';
        statusEl.className = 'idm-status success';
        ui.showToast('✓ Reset IDM trial thành công! Khởi động lại IDM để áp dụng.', 'success');
